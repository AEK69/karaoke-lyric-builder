...
function PaymentPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!s.session) { toast.error("ກະລຸນາ Login ກ່ອນ"); return; }
    ...
    <div className="min-h-screen">
      ...
      <header className="px-4 sm:px-6 pt-6 pb-4">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <h1 className="text-xl font-extrabold flex items-center gap-2"><Crown className="w-5 h-5 text-premium" /> ຊຳລະເງິນ</h1>
          <div className="w-16" />
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 pb-12 space-y-4">
        ...
        <div className="glass rounded-3xl p-5 border border-white/40 shadow-soft text-center">
          <h2 className="font-extrabold mb-3 flex items-center justify-center gap-1"><QrCode className="w-4 h-4" /> 2. ສະແກນຈ່າຍ {plan.price.toLocaleString()} ກີບ</h2>
          <img src={qr.qrCodeUrl} alt="OnePay QR" className="mx-auto w-64 h-64 bg-white rounded-2xl p-2" />
        ...
        <div className="glass rounded-3xl p-5 border border-white/40 shadow-soft">
          ...
          {slipPreview && <img src={slipPreview} alt="slip" className="mt-3 max-h-64 mx-auto rounded-xl border" />}
          <p className="text-xs text-muted-foreground mt-2">ຫຼັງຈາກກົດ "ສົ່ງສະລິບ" ລະບົບຈະເປີດ WhatsApp ໃຫ້ສົ່ງຮູບສະລິບໄປຫາແອັດມິນອັດຕະໂນມັດ (020 5866 2540)</p>
        </div>

        <Button onClick={submit} disabled={submitting} size="lg" className="w-full bg-[#25D366] hover:bg-[#1ebe5a] text-white font-bold">
        ...
        {requestId && (
          <div className="rounded-2xl bg-success/10 border-2 border-success/30 p-4 text-center">
            ...
            <div className="font-bold text-sm">ບັນທຶກຄຳຂໍແລ້ວ — Ref: <span className="font-mono">{requestId.slice(0, 8)}</span></div>
            <div className="text-xs text-muted-foreground mt-1">ແອັດມິນຈະອະນຸມັດ ແລະ ເຕີມເຄຣດິດ/Premium ໃຫ້ໃນ 24 ຊມ</div>
          </div>
        )}
      </main>
    </div>
