...
function RedeemPage() {
  ...
  const errorMsg = (e: string) =>
    e === "invalid_code"
      ? "ໂຄດບໍ່ຖືກຕ້ອງ — ກວດອີກຄັ້ງ"
      : e === "already_used" || e === "already_redeemed"
        ? "ທ່ານໃຊ້ໂຄດນີ້ໄປແລ້ວ"
        : e === "used_up"
          ? "ໂຄດນີ້ມີຄົນໃຊ້ຄົບຈຳນວນແລ້ວ"
          : e === "expired"
            ? "ໂຄດໝົດອາຍຸແລ້ວ"
            : e === "not_authenticated"
              ? "ກະລຸນາເຂົ້າສູ່ລະບົບກ່ອນໃຊ້ໂຄດ"
              : `ຜິດພາດ: ${e}`;
      ...
      <header className="px-4 sm:px-6 pt-6 pb-4">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <h1 className="text-xl font-extrabold flex items-center gap-2">
            <Gift className="w-5 h-5 text-accent" /> ໃຊ້ໂຄດເຕີມ
          </h1>
          <div className="w-16" />
        </div>
      </header>
        ...
        {!checking && !session ? (
          <div className="glass rounded-3xl shadow-soft border border-white/40 p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              ເຂົ້າສູ່ລະບົບດ້ວຍ Google ເພື່ອໃຊ້ໂຄດເຕີມ
            </p>
            <Button
              onClick={handleLogin}
              size="lg"
          ...
          <div className="glass rounded-3xl shadow-soft border border-white/40 p-6">
            ...
            <Button
              ...
              size="lg"
              className="w-full mt-3 bg-gradient-button text-primary-foreground font-bold"
            >
              {busy ? (
            ...
            {result && result.ok && (
              <div className="mt-4 rounded-2xl bg-success/10 border-2 border-success/30 p-4 text-center">
                ...
                {result.premium_days > 0 && (
                  <div className="text-sm">+{result.premium_days} ມື້ Premium</div>
                )}
                <Link to="/">
            ...
            {result && !result.ok && (
              <div className="mt-4 rounded-2xl bg-destructive/10 border-2 border-destructive/30 p-4 text-center">
                {result.error === "expired" ? (
                  <Clock className="w-10 h-10 mx-auto text-destructive mb-2" />
                ) : (
                  <X className="w-10 h-10 mx-auto text-destructive mb-2" />
                )}
